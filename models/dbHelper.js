...
function pj(obj, fields) {
  ...
  for (const f of fields) {
    if (typeof out[f] === 'string') {
      try { out[f] = JSON.parse(out[f]); } catch (e) {}
    }
  }
  return out;
}
...
const Op = {
  eq: Symbol('eq'), ne: Symbol('ne'), gte: Symbol('gte'),
  lte: Symbol('lte'), like: Symbol('like'), between: Symbol('between'),
  in: Symbol('in'), or: Symbol('or'), and: Symbol('and')
};

function isSymObj(v) {
  return v && typeof v === 'object' && Object.getOwnPropertySymbols(v).length > 0 && Object.keys(v).length === 0;
}
...
function makeModel(tableName, idName, jsonFields = []) {
  const model = {
    ...
    findAll(query = {}) {
      ...
      if (query.where) {
        ...
        for (const [k, v] of Object.entries(w)) {
          ...
          if (isSymObj(v)) {
            if (v[Op.eq] !== undefined) arr = arr.filter(x => x[k] === v[Op.eq]);
            else if (v[Op.ne] !== undefined) arr = arr.filter(x => x[k] !== v[Op.ne]);
            else if (v[Op.like] !== undefined) {
              const pat = v[Op.like].replace(/%/g, '');
              arr = arr.filter(x => String(x[k] || '').includes(pat));
            }
            else if (v[Op.gte] !== undefined) arr = arr.filter(x => x[k] >= v[Op.gte]);
            else if (v[Op.lte] !== undefined) arr = arr.filter(x => x[k] <= v[Op.lte]);
            else if (v[Op.between] !== undefined) {
              const [lo, hi] = v[Op.between];
              arr = arr.filter(x => x[k] >= lo && x[k] <= hi);
            }
            else if (v[Op.in] !== undefined) arr = arr.filter(x => v[Op.in].includes(x[k]));
          } else {
        ...
        if (w[Op.or]) {
          const ors = w[Op.or];
          arr = arr.filter(x => ors.some(cond => {
            for (const [k, v] of Object.entries(cond)) {
              if (isSymObj(v)) {
                if (v[Op.like] !== undefined) {
                  if (!String(x[k] || '').includes(v[Op.like].replace(/%/g, ''))) return false;
                } else if (v[Op.in] !== undefined) {
                  if (!v[Op.in].includes(x[k])) return false;
                } else if (v[Op.eq] !== undefined) {
                  if (x[k] !== v[Op.eq]) return false;
                }
              } else if (x[k] !== v) return false;
            }
            return true;
          }));
        }
      ...
      if (query.order) {
        for (const [f, d] of [...query.order].reverse()) {
          arr.sort((a, b) => (a[f] < b[f] ? -1 : (a[f] > b[f] ? 1 : 0)) * (d === 'ASC' ? 1 : -1));
        }
      }
      return arr;
    },
    findOne(q) { return this.findAll(q)[0] || null; },
    findByPk(id) {
      const r = db.get(tableName).find({ [idName]: Number(id) }).value();
      return r ? pj(r, jsonFields) : null;
    },
    count(q) { return this.findAll({ where: q && q.where }).length; },
    bulkCreate(items) {
      return items.map(data => {
        const id = nextId(tableName);
        const rec = sj({ [idName]: id, ...data, createdAt: nowISO(), updatedAt: nowISO() }, jsonFields);
        db.get(tableName).push(rec).write();
        return pj(rec, jsonFields);
      });
    },
    create(data) { return this.bulkCreate([data])[0]; },
    update(id, patch) {
    ...
    destroy(id) {
      ...
      db.get(tableName).remove({ [idName]: Number(id) }).write();
