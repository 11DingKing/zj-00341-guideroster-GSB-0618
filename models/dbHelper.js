...
function idEqual(a, b) {
  ...
  if (typeof b === "number" && isNumericString(a)) return b === Number(a);
  return false;
}

function idInArray(arr, val) {
  if (arr.includes(val)) return true;
  if (typeof val === "number") {
    return arr.some((item) => isNumericString(item) && Number(item) === val);
  }
  if (isNumericString(val)) {
    const numVal = Number(val);
    return arr.some((item) => typeof item === "number" && item === numVal);
  }
  return false;
}
        ...
        for (const [k, v] of Object.entries(w)) {
          ...
          if (isSymObj(v)) {
            if (v[Op.eq] !== undefined)
              arr = arr.filter((x) => idEqual(x[k], v[Op.eq]));
            else if (v[Op.ne] !== undefined)
              arr = arr.filter((x) => !idEqual(x[k], v[Op.ne]));
              ...
              arr = arr.filter((x) => idInArray(v[Op.in], x[k]));
          } else {
            arr = arr.filter((x) => idEqual(x[k], v));
          }
        }
        if (w[Op.or]) {
          const ors = w[Op.or];
              ...
              for (const [k, v] of Object.entries(cond)) {
                if (isSymObj(v)) {
                  if (v[Op.like] !== undefined) {
                    ...
                    if (!idInArray(v[Op.in], x[k])) return false;
                  } else if (v[Op.eq] !== undefined) {
                    if (!idEqual(x[k], v[Op.eq])) return false;
                  }
                } else if (!idEqual(x[k], v)) return false;
              }
              return true;
            }),
          );
